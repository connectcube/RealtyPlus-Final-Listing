...
const NearbyPlaces = ({
  ...
  const [selectedPlaces, setSelectedPlaces] = useState<Set<number>>(() => {
    // Initialize with existing places from formData
    return new Set(formData.nearbyPlaces.map((place) => place.id));
  });
  ...
  const allPlaces = [
    ...
    ...fetchedPlaces.filter(
      (place) =>
        !formData.nearbyPlaces.some((existing) => existing.id === place.id)
    ),
  ];
